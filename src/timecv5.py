...
"""Rolling time-series cross-validation with fold count and anomaly constraints."""
...
def _rolling_splits_with_min_anoms(
    ...
    if max_possible == 0:
        return []
    return splits[:max_possible]


def main() -> None:
    run_dir = make_run_dir("timecv5")
    ...
    summary["min_test_anoms"] = 5

    (run_dir / "metrics_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")

    report_lines = []
    report_lines.append("# Rolling time-CV report (5 folds max)")
    report_lines.append("")
    report_lines.append(f"- data_path: {data_path}")
    report_lines.append("- feature_set: A (no anomaly_* and no target-like columns)")
    report_lines.append("- model: GradientBoostingClassifier")
    report_lines.append(f"- folds_used: {len(rows)}")
    report_lines.append("- min_test_anoms: 5")
    report_lines.append("")
    ...
    print(f"OK TimeCV5 run saved to {run_dir}")
